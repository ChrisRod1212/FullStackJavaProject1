...
function createReimbursement(reimbursement) {
  ...
  <div id="reimb-container">
        ...
        <div class="container-row" id="approveDeny">
            <div>
                <button class="btn" id="approve" onclick="approve(${reimbursement.reimbId});">Approve</button>
                <button class="btn" id="deny" onclick="deny(${reimbursement.reimbId});">Deny</button>
            </div>
          </div>
      </div>
...
async function deny(element){
  ...
  console.log(reimbId)
  let response = await fetch("http://localhost:9001/reimbursement/"+elementreimbId, {
      method: "PATCH",
      body: JSON.stringify(reimbursement)  
  })
